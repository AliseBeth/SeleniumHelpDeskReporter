...
link2 = "путь к выгруженному файлу отчета, который будет проверяться алгоритмом"
a = 0
...
browser.get(link2)
table_body = browser.find_element(By.XPATH, "//table[@style='width: 735px; border-collapse: collapse; empty-cells: show'] /tbody")
for link in table_body.find_elements(By.XPATH, "//span[@style='font-family: Roboto,Arial,Helvetica,sans-serif; color: #0000FF; font-size: 10px; line-height: 1.1640625;']"):
    ...
    try:
        ...
        specialist = browser.find_element(By.XPATH,
                                          "// div[@id='technician-right-panel'] /p[@class='form-control-static spot-static colon']").text
                ...
                if looking_for_emails == []:
                    if 'claim@email.ru' in user_1:
                        ...
                        if number_emails > 1:
                            ...
                            print(request_id, user_1, user_2, "------", 'ПОСТАВИЛИ ХЭЛПДЕСК В КОПИЮ ПИСЬМА')
                            break
                else:
                    if 'claim@email.ru' in user_1:
                        number_emails = user_1.count('help@email.ru') + 1
        ...
        print(request_id, '----- Что-то не так с заявкой')
